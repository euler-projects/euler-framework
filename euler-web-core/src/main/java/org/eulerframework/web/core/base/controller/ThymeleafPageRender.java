...
import org.eulerframework.web.util.TemplateTextUtils;
    ...
    private Map<String, Object> newModelInstance() {
        if (TEMPLATE_ATTRIBUTES == null) {
            synchronized (TEMPLATE_ATTRIBUTES_LOCK) {
                if (TEMPLATE_ATTRIBUTES == null) {
                    // ctx  - the servlet context's euler attributes
                    // text - text helpers a template may need while
                    //        rendering, e.g. spacing mixed-script text
                    TEMPLATE_ATTRIBUTES = Map.of(
                            "ctx", getEulerAttributesContext(),
                            "text", TemplateTextUtils.INSTANCE);
                }
            }
        }
