...
import org.eulerframework.security.oauth2.server.authorization.authentication.OAuth2AppleAppAttestAttestationAuthenticationToken;
...
public class OAuth2AppleAppAttestAttestationAuthenticationConverter implements AuthenticationConverter {
    ...
    public Authentication convert(HttpServletRequest request) {
        ...
        if (!EulerAuthorizationGrantType.APPLE_APP_ATTEST_ATTESTATION.getValue().equals(grantType)) {
            return null;
        }
        ...
        String attestation = parameters.getFirst(EulerOAuth2ParameterNames.ATTESTATION);
        if (!StringUtils.hasText(attestation) || parameters.get(EulerOAuth2ParameterNames.ATTESTATION).size() != 1) {
            throw new OAuth2AuthenticationException(new OAuth2Error(OAuth2ErrorCodes.INVALID_REQUEST,
                    "OAuth 2.0 Parameter: " + EulerOAuth2ParameterNames.ATTESTATION, DEFAULT_ERROR_URI));
        }
        ...
        parameters.forEach((key, value) -> {
            if (!key.equals(OAuth2ParameterNames.GRANT_TYPE) &&
                    ...
                    !key.equals(EulerOAuth2ParameterNames.ATTESTATION) &&
                    !key.equals(EulerOAuth2ParameterNames.CHALLENGE)) {
                additionalParameters.put(key, (value.size() == 1) ? value.get(0) : value.toArray(new String[0]));
            }
        });

        return new OAuth2AppleAppAttestAttestationAuthenticationToken(
                keyId,
                attestation,
                challenge,
                clientPrincipal,
