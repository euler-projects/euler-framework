 ...
 * @author Dave Syer
 */
public class EulerJdbcTokenStore implements TokenStore {

    private static final Log LOG = LogFactory.getLog(EulerJdbcTokenStore.class);
    ...
    public EulerJdbcTokenStore(DataSource dataSource) {
    ...
    public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
        ...
        try {
            ...
            String[] keys = {key};
            List<OAuth2AccessToken> results = jdbcTemplate.query(selectAccessTokenFromAuthenticationSql, keys, 
                    new RowMapperResultSetExtractor<OAuth2AccessToken>(new RowMapper<OAuth2AccessToken>() {
                        public OAuth2AccessToken mapRow(ResultSet rs, int rowNum) throws SQLException {
                            return deserializeAccessToken(rs.getBytes(2));
                        }
                    }));
            if(!results.isEmpty()) {
                for(OAuth2AccessToken existingAccessToken : results) {
