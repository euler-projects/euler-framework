...
public class FileUploadAndDownloadWebController extends JspSupportWebController {
    ...
    public void downloadArchivedFile(@PathVariable("param") String param) throws IOException {
    ...
    public void downloadImage(@PathVariable("param") String param,
        ...
        if (extensions != null) {
            if (!extensions.equalsIgnoreCase(archivedFile.getExtension())) {
                if (this.logger.isInfoEnabled()) {
                    this.logger.info("The file extension does not match, specified as " + extensions + ", actually "
                            + archivedFile.getExtension());
                }
                throw new ResourceNotFoundException();
            }
        }
    ...
    protected void writeImage(String fileName, File file, double scale) throws FileNotFoundException, IOException {
        ...
        response.setHeader("Content-Disposition", mime.getContentDisposition() + ";fileName=\""
                + new String(fileName.getBytes("utf-8"), "ISO8859-1") + "\"");
        // response.setHeader("Transfer-Encoding", "chunked");
        // response.setHeader("Content-Length", String.valueOf(file.length()));

        if (scale != 1) {
            Thumbnails.of(file).scale(scale).outputQuality(1f).toOutputStream(response.getOutputStream());
        } else {
    ...
    public EasyUIAjaxResponse<ArchivedFile> uploadArchivedFile(
            @RequestParam(value = "file") MultipartFile multipartFile) throws FileArchiveException {
