...
class DelegatingOtpChannelTest {
    ...
    void supportsReflectsRoutesAndFallback() {
        DelegatingOtpChannel noFallback = new DelegatingOtpChannel(Map.of("sms", channel("sms")));
        DelegatingOtpChannel withFallback = new DelegatingOtpChannel(Map.of(), new StdoutOtpChannel());
    ...
    void routingIsCaseInsensitive() {
        DelegatingOtpChannel delegating = new DelegatingOtpChannel(Map.of("SMS", channel("sms")));
    ...
    void routesDifferingOnlyInCaseAreRejected() {
        Map<String, OtpChannel> routes = Map.of(
                "sms", channel("sms"),
                "SMS", channel("sms"));

        Assertions.assertThrows(IllegalArgumentException.class, () -> new DelegatingOtpChannel(routes));
    }

    @Test
    void routeMissWithoutFallbackThrowsSynchronously() {
        DelegatingOtpChannel delegating = new DelegatingOtpChannel(Map.of("email", channel("email")));

        Assertions.assertThrows(OtpChannelNotFoundException.class, () -> delegating.send(DELIVERING));
    }
